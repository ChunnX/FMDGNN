...
def baseline_cross_validation(cross_datasets, global_test_dataset, n_folds, res):
    ...
    for fold in range(n_folds):
        ...
        # Evaluate each fold
        test_loss_1, _, topo_1 = evaluate(model_1, test_loader_1, device)
        test_loss_2, _, topo_2 = evaluate(model_2, test_loader_2, device)
        test_loss_3, _, topo_3 = evaluate(model_3, test_loader_3, device)
        fold_test_loss_1.append(test_loss_1)
        fold_test_loss_2.append(test_loss_2)
        fold_test_loss_3.append(test_loss_3)
        fold_topo_1.append(topo_1)
        fold_topo_2.append(topo_2)
        fold_topo_3.append(topo_3)

        global_test_loader = DataLoader(global_test_dataset)
        global_test_loss_1, _, global_topo_1 = evaluate(model_1, global_test_loader, device)
        global_test_loss_2, _, global_topo_2 = evaluate(model_2, global_test_loader, device)
        global_test_loss_3, _, global_topo_3 = evaluate(model_3, global_test_loader, device)
        ...
        fold_global_topo_1.append(global_topo_1)
        fold_global_topo_2.append(global_topo_2)
        fold_global_topo_3.append(global_topo_3)
    ...
    fold_topo_loss = [fold_topo_1, fold_topo_2, fold_topo_3]
    global_eval_loss = [fold_global_test_loss_1, fold_global_test_loss_2, fold_global_test_loss_3]
    global_topo_loss = [fold_global_topo_1, fold_global_topo_2, fold_global_topo_3]

    return best_models, fold_train_loss_log, fold_eval_loss, fold_topo_loss, global_eval_loss, global_topo_loss
