...
def train(hospital_list, local_datasets, local_tests,
    ...
    for i in range(len(hospital_list)):
        client = Client(hospital_list[i], optimizer_list[i],
                        ...
                        c_idx=c_idx, lamb=lamb, kl=kl, topo=topo)
    ...
    for i in range(len(client_list)):
        eval_loss, view_loss, topology_loss = evaluate(client_list[i].model, test_loader_list[i], device)
        eval_loss_list.append(eval_loss)
        view_loss_list.append(view_loss)
        topology_list.append(topology_loss)

    global_eval_loss, global_view_loss, global_topo = evaluate(server.global_model, global_test_loader, device)

    local_eval = [eval_loss_list, view_loss_list, topology_list]
    global_eval = [global_eval_loss, global_view_loss, global_topo]
